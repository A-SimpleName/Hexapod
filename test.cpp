...
void loop() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        //servo [i][1] = legAngles[i][0];
        //servo [i][2] = legAngles[i][1];
        //servo [i][3] = legAngles[i][2];
    }
...
Point3d getPointBezier(Point3d P1, Point3d P2, double stepHeight) {
    ...
    double dist_P1_P2 = sqrt( pow(P1.x - P2.x, 2) + pow(P1.y - P2.y, 2) + pow(P1.z - P2.z, 2) );

    p1 = {0, height};
    p3 = {1, height};
    p2 = {0.5, stepHeight / dist_P1_P2};
